package xyz.techiecrow.world.gen;
...
public class Oregen implements IWorldGenerator
{
	...
	public void generateInNether(World world, Random random, int x, int y)
	{
		for (int i = 0; i < 1; i++)
		{
			...
			new NetherGenMinable(xyz.techiecrow.init.ModBlockOres.ShadowBoronOre, ConfigurationHandler.oreShadowBoronSpawnsPerVein).generate(world, random, x1coord, y1coord, z1coord);

			int x2coord = x + random.nextInt(16);
			int y2coord = random.nextInt(ConfigurationHandler.oreHeavenlyGlintSpawnHeight);
			int z2coord = y + random.nextInt(16);
			new NetherGenMinable(xyz.techiecrow.init.ModBlockOres.HeavenlyGlintOre, ConfigurationHandler.oreHeavenlyGlintSpawnsPerVein).generate(world, random, x2coord, y2coord, z2coord);
			// System.out.println("Spawned at: " + world + " " + xcoord + " " + ycoord + " " + zcoord);
		}
	}
}