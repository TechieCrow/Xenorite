package ninja.techiecrow.world.gen;
...
import ninja.techiecrow.world.gen.feature.NetherGenMinable;
...
public class Oregen implements IWorldGenerator
{
	...
	public void generateInOverworld(World world, Random random, int x, int z)
	{
		for (int i = 0; i < 1; i++) // The number after i (in this case 6) is how many veins spawn per chunk (16*16*16 area)
		{
			...
			new WorldGenMinable(ninja.techiecrow.init.ModBlockOres.xenoriteOre, ConfigurationHandler.oreXenoriteSpawnsPerVein).generate(world, random, xcoord, ycoord, zcoord);
			...
			new WorldGenMinable(ninja.techiecrow.init.ModBlockOres.coreoriteOre, ConfigurationHandler.oreCoreoriteSpawnsPerVein).generate(world, random, xcoord, ycoord, zcoord);
			...
			new WorldGenMinable(ninja.techiecrow.init.ModBlockOres.finoriteOre, ConfigurationHandler.oreFinoriteSpawnsPerVein).generate(world, random, xcoord, ycoord, zcoord);
	...
	public void generateInNether(World world, Random random, int x, int y)
	{
		for (int i = 0; i < 1; i++)
		{
			...
			new NetherGenMinable(ninja.techiecrow.init.ModBlockOres.ShadowBoronOre, ConfigurationHandler.oreShadowBoronSpawnsPerVein).generate(world, random, x1coord, y1coord, z1coord);

			int x2coord = x + random.nextInt(16);
			int y2coord = random.nextInt(ConfigurationHandler.oreHeavenlyGlintSpawnHeight);
			int z2coord = y + random.nextInt(16);
			new NetherGenMinable(ninja.techiecrow.init.ModBlockOres.HeavenlyGlintOre, ConfigurationHandler.oreHeavenlyGlintSpawnsPerVein).generate(world, random, x2coord, y2coord, z2coord);
			// System.out.println("Spawned at: " + world + " " + xcoord + " " + ycoord + " " + zcoord);
		}
	}
}